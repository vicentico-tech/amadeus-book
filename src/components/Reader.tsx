...
export function Reader({ book, onBack }: { book: Book; onBack: () => void }) {
  ...
  const goTo = useCallback(
    ...
    [book.id, book.pageCount, pageNumber],
  );
  ...
  const pageWidth = showSecondPage ? Math.max((containerWidth - 1) / 2, 0) : containerWidth;

  return (
    <div className="min-h-screen bg-canvas text-ink">
      <header
        className={`flex flex-col gap-2 border-b border-line px-[var(--pad-container)] py-4 transition-opacity duration-200 sm:flex-row sm:items-center sm:justify-between ${controlsVisible ? "opacity-100" : "pointer-events-none opacity-0"
          }`}
      >
        <div className="flex min-w-0 items-center gap-3">
          <button onClick={onBack} className="shrink-0 font-mono text-[11px] uppercase tracking-[0.1em] text-ink-muted">
            ← Biblioteca
          </button>
          <h2 className="min-w-0 flex-1 truncate font-serif text-lg">{book.title}</h2>
        </div>
        <div className="flex items-center justify-between gap-4 font-mono text-[11px] uppercase tracking-[0.1em] text-ink-muted sm:justify-end">
          <span>
            {pageNumber}
      ...
      <main className="flex flex-col items-center gap-4 px-[var(--pad-container)] py-8">
        <motion.div
          ref={containerRef}
        ...
        <div
          className={`flex w-full max-w-[560px] flex-col items-center gap-3 px-[var(--pad-container)] transition-opacity duration-200 ${controlsVisible ? "opacity-100" : "pointer-events-none opacity-0"
            }`}
        >
            ...
            {toc === null ? (
              ...
              toc.map((entry, i) => (
                <button
                  ...
                  className={`flex items-center justify-between rounded-sm px-2 py-2 text-left text-sm disabled:text-ink-muted ${entry.page !== null && (pageNumber === entry.page || pageNumber + 1 === entry.page)
